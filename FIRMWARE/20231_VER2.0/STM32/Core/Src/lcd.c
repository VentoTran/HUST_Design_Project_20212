...
static void ILI9341_WriteData(uint8_t *buff, size_t buff_size)
{
    ...
    while (buff_size > 0)
    {
        DATA_PORT->ODR = (uint32_t)buff[dataIdx];
        HAL_GPIO_WritePin(ILI9341_WR_GPIO_Port, ILI9341_WR_Pin, GPIO_PIN_RESET);
        HAL_GPIO_WritePin(ILI9341_WR_GPIO_Port, ILI9341_WR_Pin, GPIO_PIN_SET);
        buff_size--;
        dataIdx++;
...
#if USE_16BIT == 1
static void ILI9341_WriteData16Bit(uint16_t data)
{
    HAL_GPIO_WritePin(ILI9341_DC_GPIO_Port, ILI9341_DC_Pin, GPIO_PIN_SET);
    DATA_PORT->ODR = (uint32_t)data;
    HAL_GPIO_WritePin(ILI9341_WR_GPIO_Port, ILI9341_WR_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(ILI9341_WR_GPIO_Port, ILI9341_WR_Pin, GPIO_PIN_SET);
}
#endif
    ...
    for (i = 0; i < font.height; i++)
    {
        ...
        for (j = 0; j < font.width; j++)
        {
            ...
            else
            {
                ...
                ILI9341_WriteData(data, sizeof(data));
#else
                ILI9341_WriteData16Bit(bgcolor);
#endif
            }
        }
    }
...
#if USE_16BIT == 1
    int data2write = width * height;
    for (int i = 0; i < data2write; i++)
    {
        ILI9341_WriteData16Bit(pdata[i]);
    }
#else
